#!/usr/bin/env python3
# DarkSec Logger – Web (Mobile-Optimized) Edition
# Features: Flask admin (responsive), Proxy-IP, Bot Filter, AES-256 enc/dec, LocalTunnel
...
def parse_lt_url(line: str) -> Optional[str]:
...
<style>
  ...
  .mono { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 12px; white-space: pre-wrap; background:#0d0d0d; border:1px solid #222; padding:10px; border-radius:8px; height: 240px; overflow:auto; }
...
def home():
    return render_template_string(
        ...
        lt_url=lt_url,
        public_base=(lt_url or f"http://{request.host}"),
        doctor=None
    )
...
def lt_start():
    ...
    # read a few lines async-ish to capture URL
    for _ in range(100):
        line = lt_proc.stdout.readline()
        if not line: break
        add_log(line.strip())
        url = parse_lt_url(line)
        if url:
            lt_url = url
...
def doctor_report() -> str:
    ...
    # Flask running implies network & web UI ok
    ...
    else: warn("node missing → macOS: brew install node | Debian: sudo apt install nodejs")

    if npm: ok(f"npm found: {npm}")
    else: warn("npm missing → macOS: brew install npm | Debian: sudo apt install npm")

    if lt: ok(f"localtunnel found: {lt}")
    else: warn("LocalTunnel missing → npm install -g localtunnel")

    if selected_image:
        ok(f"Image selected: {selected_image.name}")
    else:
        warn("No image selected yet.")

    if lt_url:
        ok(f"LocalTunnel active: {lt_url}")
    else:
        warn("LocalTunnel not running.")
