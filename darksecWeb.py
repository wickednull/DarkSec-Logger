...
def track_image(fname):
    ...
    add_log(
        f"[{tag}] IP: {client_ip} | UA: {ua} | PATH: {request.path}?{request.query_string.decode()} | ID: {click_id}")
...
def cf_start():
    ...
    if not cf_bin:
        add_log(
            "[WARN] cloudflared not found. Install: macOS 'brew install cloudflared' · Debian/Ubuntu 'sudo apt install cloudflared'")
        return redirect(url_for("home"))
...
def doctor_report() -> str:
    ...
    def warn(s):
        msgs.append(f"[WARN] {s}")

    def fail(s):
        msgs.append(f"[FAIL] {s}")

    ok(f"Python {sys.version.split()[0]}")
    if HAS_CRYPTO:
        ok("PyCryptodome installed")
    else:
        fail("PyCryptodome missing → python3 -m pip install pycryptodome")
    ...
    if node:
        ok(f"node found: {node}")
    else:
        warn("node missing → macOS: brew install node | Debian/Ubuntu: sudo apt install nodejs")
    if npm:
        ok(f"npm found: {npm}")
    else:
        warn("npm missing → macOS: brew install npm | Debian/Ubuntu: sudo apt install npm")

    if lt:
        ok(f"localtunnel found: {lt}")
    else:
        warn("LocalTunnel missing → npm install -g localtunnel")

    if cf:
        ok(f"cloudflared found: {cf}")
    else:
        warn("cloudflared missing → macOS: brew install cloudflared | Debian/Ubuntu: sudo apt install cloudflared")
    ...
    if cf_url:
        ok(f"Cloudflare active: {cf_url}")
    else:
        warn("Cloudflare not running.")
