#!/usr/bin/env python3
# DarkSec Logger – LocalTunnel + Cloudflare Tunnel + Doctor (Audited Build)
# Features: Proxy-IP, Bot Filter, AES-256, GUI/Silent, 2-row toolbar, LT+ngrok+Cloudflare
...
APP_TITLE = "DarkSec Logger – LT + CF + Doctor"
DEFAULT_PORT = 8000
...
def stop_logging():
    global server, ngrok_tunnel, lt_proc, cf_proc
    if server:
...
def start_cloudflare_tunnel():
    ...
    if not cf_bin:
        _log_gui_or_print(
            ...
            "  Linux (Debian/Ubuntu): sudo apt install cloudflared  (or see Cloudflare docs)\n"
    ...
    def reader():
        ...
        for line in cf_proc.stdout:
            _log_gui_or_print(line)
            # TryCloudflare prints a URL ending with trycloudflare.com
...
def run_doctor():
    ...
    else: msgs.append(warn("node missing → macOS: brew install node | Debian: sudo apt install nodejs"))
    if npm: msgs.append(ok("npm found"))
    else: msgs.append(warn("npm missing → macOS: brew install npm | Debian: sudo apt install npm"))
    ...
    else: msgs.append(warn("cloudflared missing → macOS: brew install cloudflared | Debian/Ubuntu: sudo apt install cloudflared (or see Cloudflare docs)"))
...
Usage:
  ...
  python3 DarkSecLogger.py --silent --cloudflare
